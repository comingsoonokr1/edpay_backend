...
export class SafeHavenProvider {
    ...
    static async getAirtimeProviders() {
        ...
        const airtimeService = servicesRes.data.find((s) => s.name.toLowerCase() === "airtime");
        if (!airtimeService)
        ...
        const providers = await Promise.all(categories.map(async (cat) => {
    ...
    static async getDataProviders() {
        ...
        console.log(servicesRes);
        const dataService = servicesRes.data.find((s) => s.name.toLowerCase() === "data purchase");
        if (!dataService)
 ...
 */
    static async getVASProviders(category) {
        ...
        try {
            ...
            console.log(servicesRes);
            const service = servicesRes.data.find((s) => s.name.toLowerCase().includes(category));
            if (!service)
