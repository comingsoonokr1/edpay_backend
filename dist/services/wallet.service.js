...
export class WalletService {
    ...
    static async getTransactionByReference(userId, reference) {
    ...
    static async withdraw(userId, amount) {
        ...
        try {
            ...
            if (wallet.balance < amount)
                throw new ApiError(403, "Insufficient funds");
    ...
    static async transfer({ senderId, recipient, amount, bankName, transactionPin, note, }) {
        if (amount <= 0)
        ...
        try {
            ...
            const isPinValid = await comparePassword(transactionPin, sender.transactionPin);
            if (!isPinValid)
