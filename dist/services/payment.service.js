...
export class PaymentService {
    ...
    static async verifyPayment(reference) {
        ...
        const paymentIntent = await stripe.paymentIntents.retrieve(transaction.meta.paymentIntentId);
        if (paymentIntent.status !== "succeeded") {
            throw new ApiError(400, "Payment not completed");
        }
        // Atomic wallet update
        const wallet = await Wallet.findOneAndUpdate({ userId: transaction.userId }, { $inc: { balance: transaction.amount } }, { new: true });
        if (!wallet) {
    ...
    static async listTransactions(filter) {
